...
class Context():
    ...
    def get_indicator_vector(self, LABEL_MAP):
        uniq_labl_counts = np.unique(LABEL_MAP, return_counts=True)
        uniq_labl = uniq_labl_counts[0]

        # fixed thresholding on number of pixels to presents for considering it as label
       	#uniq_labl = uniq_labl[uniq_labl_counts[1] > 100]
...
#                pdb.set_trace()
                XMP_MAP = cv2.imread(
                    self.CACHE_EXEMPLAR_PATH + EXEMPLARS_LIST[n].decode('UTF-8'))
